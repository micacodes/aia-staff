...
/**
 * Key points
 * Below endpoints are needed:
 * An endpoint to fetch all customers (based on certain criteria)
 * Callback functions to:
 * 1. Send SMS
 * 2. Send Email
 * 3. Send messages via WhatsApp
 * 4. Send in-app messages.
 * Note: A staff can broadcast messages to more than one channel.
 */
...
export default function BroadcastPage({ navigation }) {
  ...
  const [templates, setTemplates] = useState<
    { key: string | number; value: string; content: string }[]
  >([]);
  ...
  useEffect(() => {
    if (session?.branch?.id || session?.vendor?.id || session?.id) {
      ...
      ); // Avoid overwriting user input unnecessarily unless intended
    }
  }, [session, reset]);
  ...
  const loadMessages = useCallback(() => {
    ...
    api
      .get<Message[]>("messages") // Expect Paginated or Array  //Changed from ListApiResponse<Message>
      .then(({ data: responseData }) => {
        // data has changed
        console.log(
          "[BroadcastPage] loadMessages: RAW Response:",
          responseData
        );
        // Handle both direct array and paginated {data: [...]}
        if (Array.isArray(responseData)) {
          console.log(
            `[BroadcastPage] loadMessages: Processing ${responseData.length} messages.`
          );
          setMessages(responseData); // Changed
        } else {
          console.warn(
            "[BroadcastPage] loadMessages: Received invalid structure:",
            responseData
          );
      ...
      .catch((error) => {
        console.error(
          "[BroadcastPage] loadMessages: Error fetching messages:",
          error
        );
  ...
  const loadGroups = useCallback(() => {
    ...
    if (session?.branch?.id && session?.id) {
      api
        .get<CustomerGroup[]>(`groups/${session?.id}`) // Use the new endpoint.  // Changed from ListApiResponse<CustomerGroup>
        .then(({ data: responseData }) => {
          // Changed
          console.log("[BroadcastPage] Groups API Response:", responseData);
          if (Array.isArray(responseData)) {
            console.log(
              `[BroadcastPage] Processing ${responseData.length} groups.`
            ); // Changed
            setCustomerGroups(responseData); // Store the fetched groups
            setGroups(responseData.map((g) => ({ key: g.id, value: g.name }))); // Changed
          } else {
            console.warn(
              "[BroadcastPage] Invalid structure for groups:",
              responseData
            );
            setCustomerGroups([]);
            setGroups([]);
  ...
  const sendMessage = async (formData: BroadcastFormData) => {
    // Receive validated data from handleSubmit
    // Client-side validation (already good)
    if (!formData.type || !formData.message || !formData.channels?.length) {
      Alert.alert("Missing Info", "Type, message, and channel(s) required.");
      return;
    }
    if (formData.type === "groups" && !formData.groups?.length) {
      Alert.alert("Missing Info", "Select at least one group.");
      return;
    }
    if (formData.type === "individual" && !formData.receipients?.length) {
      Alert.alert("Missing Info", "Select at least one recipient.");
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Success",
        `Broadcast message ${formData.id ? "resent" : "sent"} successfully.`
      );
      ...
      Alert.alert(
        "Error",
        `Failed to ${formData.id ? "resend" : "send"} broadcast message.`
      );
      if (error.response) {
  ...
  useEffect(() => {
    ...
    const canCreateTemplates =
      session?.role === "admin" || session?.role === "vendor_admin";
    ...
    if (session?.branch?.id && session?.id) {
      ...
      api
        .get<MessageTemplate[]>(templateFetchURL)
        .then(({ data: responseData }) => {
          // Changed from ListApiResponse<MessageTemplate>
          console.log("[BroadcastPage] Templates API Response:", responseData);
          if (Array.isArray(responseData)) {
            console.log(
              `[BroadcastPage] Processing ${responseData.length} templates.`
            );
            setTemplates(
              responseData.map((t) => ({
                key: t.id,
            ...
            console.warn(
              "[BroadcastPage] Invalid structure for templates:",
              responseData
            );
            setTemplates([]);
          }
        ...
        .catch((error) => {
          console.error("[BroadcastPage] Error fetching templates:", error);
          setTemplates([]);
        });
      ...
      setMessages([]);
      setTemplates([]);
      setCustomers([]); // Clear data if session is missing
    }
  ...
  const renderMessageItem = useCallback(
    ({ item: msg }: { item: Message }) => {
      ...
      let formattedMonth = "ERR";
      let formattedDay = "ERR";
      try {
        ...
        <View className="bg-white rounded-2xl border border-primary-600 mb-2 overflow-hidden shadow-sm">
          ...
          <View className="flex flex-row py-3 px-2 space-x-2">
            ...
            <View className="w-1/5 border border-primary-600 rounded-lg flex justify-center items-center py-2 aspect-square bg-gray-50">
              <Text
                className="font-bold text-primary-700 text-uppercase text-base"
          ...
          <View className="flex flex-row w-full border-t border-gray-200">
            <TouchableOpacity
              onPress={() => {
                console.log(
                  "[BroadcastPage] Resend button pressed for message:",
                  msg.id
                );
                ...
                if (msg?.template?.id && msg.template?.content) {
                  ...
                  console.warn(
                    "[BroadcastPage] Cannot resend message, missing data:",
                    msg
                  );
                  Alert.alert(
                    "Error",
                    "Cannot resend, message data incomplete."
                  );
                }
              }}
    ...
    [session, reset]
  ); // Added dependencies used inside
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <View style={styles.centered}>
            <Text style={styles.infoText}>No broadcast messages found.</Text>
          </View>
        )}
      />
              ...
              <View className="flex-1 px-4 pt-4 pb-8">
                ...
                <View className="flex flex-row justify-between items-center mb-6 pb-2 border-b border-gray-300">
                  <Text className="text-xl font-bold text-gray-700">
                    {broadcast.id ? "Resend Broadcast" : "Create New Broadcast"}
                  </Text>
                  <Pressable
                ...
                <View style={styles.formGroup}>
                  ...
                  <SelectList
                    ...
                    data={[
                      ...
                      { key: "groups", value: "Customer Groups" },
                      { key: "individual", value: "Individual Customers" },
                    ]}
                    save="key"
                ...
                {broadcast.type === "individual" && (
                  <View style={styles.formGroup}>
                    ...
                    <Text style={styles.infoText}>
                      (Customer selection not yet implemented)
                    </Text>
                    {/* <MultipleSelectList
                ...
                <View style={styles.formGroup}>
                  ...
                  <MultipleSelectList
                    setSelected={(selectedChannels) =>
                      setValue("channels", selectedChannels as string[])
                    }
                    data={[
                      { key: "sms", value: "SMS" },
                      { key: "mail", value: "Email" },
                      { key: "whatsapp", value: "WhatsApp" },
                      { key: "in-app", value: "In-App" },
                    ]} // Added in-app
                    save="key"
                    label="Channels"
                    ...
                    badgeStyles={styles.badge}
                    // Set default based on form state
                    defaultOptions={(Array.isArray(broadcast.channels)
                      ? broadcast.channels
                      : []
                    ).map((key) => ({
                      key,
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <TextInput
                      ...
                      editable={
                        ...
                        !!broadcast.id
                      } // Read-only for staff/manager, editable for admin/vendor admin, edit when resending
                ...
                >
                  {isSubmitting ? (
                    ...
                    <Icon
                      name={broadcast.id ? "history" : "send"}
                      size={18}
                      color="white"
                      style={styles.buttonIcon}
                    />
